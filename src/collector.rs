...
pub async fn collect_stats() -> SystemStats {
    ...
    // 修复1: 使用 refresh_cpu() 而不是 refresh_cpu_all()
    sys.refresh_cpu();
        ...
        Ok(nvml) => {
            ...
            match nvml.device_by_index(0) {
                Ok(device) => {
                    ...
                    // 功耗（毫瓦转瓦特）
                    let power_draw_watts = device.power_usage().map(|p| p / 1000).unwrap_or(0);
...
fn collect_battery_info() -> Option<BatteryInfo> {
    ...
    // 修复2: battery crate 返回 Quantity 类型，需要 .value 获取 f32
